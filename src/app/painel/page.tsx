...
export default async function DashboardPage() {
  ...
  return <AccountShell admin={session.user.role === "ADMIN"}>
    ...
    {status === "ACTIVE" && <div className="panel-card"><h3>Seu Plano Pro está ativo até {subscription?.expiresAt?.toLocaleDateString("pt-BR")}</h3><p>Baixe o instalador, instale no computador do caixa e entre com este mesmo e-mail e senha.</p><div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}><a className="button button-primary" href="/downloads/SistemaDeCaixa-Setup.exe" download>Baixar o Sistema de Caixa (Windows)</a><form action={accessSystemAction}><button className="button button-ghost">Acessar o Sistema de Caixa</button></form></div></div>}
